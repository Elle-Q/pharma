...
public class PrescriptionProcessService {
    ...
    public PrescriptionActionResult fulfill(Long prescriptionId) {
        ...
        try {
            ...
            context.addFailureReason(FailureReason.fail0(FailureCode.CONCURRENT_MODIFICATION, e.getMessage()));
        } catch (Exception e) {
            //system error
            context.addFailureReason(FailureReason.fail0(FailureCode.SYSTEM_ERROR, e.getMessage()));
            e.printStackTrace();
        }
